...
namespace Geometry
{
    ...
    /// </summary>
    public class PolySetVertexEnum(IReadOnlyList<Polygon> polys, int iStartingPolyIndex = 0) : IEnumerator<PolygonIndex>, IEnumerator, IEnumerable<PolygonIndex>, IEnumerable
    {
        PolygonIndex? curIndex = new Geometry.PolygonIndex?();

        readonly IReadOnlyList<Polygon> polygons = polys;
        ...
        private PolygonIndex? NextIndex(IReadOnlyList<Polygon> inputPolygons, PolygonIndex current)
        {
            int iPoly = current.ShapeIndex - StartingPolyIndex;
            Polygon poly = inputPolygons[iPoly];
            ...
            if (iNextVert < ring.Length - 1) //-1 because we do not want to report a duplicate vertex for a closed ring
            {
                ...
                return new PolygonIndex(current.ShapeIndex, current.InnerShapeIndex, iNextVert, ring.Length - 1);
            }
