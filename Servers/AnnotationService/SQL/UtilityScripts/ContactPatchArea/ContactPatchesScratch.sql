...
select SParentID, TParentID, SZ, NumPairs from (Select SParentID, TParentID, SZ, Count(SParentID) as NumPairs from #StructureLinks group by SParentID, TParentID, SZ) as ExtraStructureLinks where NumPairs > 1 order by NumPairs, SParentID
...
select S.ParentID as SourceParentID, SL.SParentID as SourceID, T.ParentID as TargetParentID, SL.TParentID as TargetID, S.TypeID as SourceTypeID, T.TypeID as TargetTypeID, sum( Line.STLength()) * @AreaScalar as Area_nm, geometry::CollectionAggregate(Shape) as shape from #StructureLinks SL
