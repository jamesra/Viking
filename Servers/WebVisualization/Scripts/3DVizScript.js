...
function RunAfterLoad() {  
    ...
    $("#freshQuery").qtip({
        content: { text: 'Update 3D model on server by generating a fresh mesh' },
        position: {
            corner: {
                tooltip: 'bottomMiddle', // Use the corner.
                target: 'topMiddle' // ...and opposite corner
...
function legit(cellID) {

    var ansArray = cellID.split(' ');

    var result = true;

    for (var i = 0; i < ansArray.length; i++) {
        result = result & (actual.indexOf(cellID) != -1);
...
function update_button() {
    ...
    if (cellID.length > 0 ) {
        ...
        document.getElementById("message").innerHTML = " <b style='color: #0000FF'>Hit Go! to generate 3D Plot</b> ";
        
    }

    else {
       
        document.getElementById("message").innerHTML = " <b style='color: #E41B17'>Enter / Choose Valid Cell (s)</b> ";
    ...
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
        ...
        if (xmlHttp.responseText == "Not found") {
            ...
            for (key in structureIDMap) {
                ...
                structureData.push({ id: key, type: structureIDMap[key].replace(',', ''), count: structureIDSort[key] });
            }
