...
--from (
select  SideA_Child1_L1.ID       as  SideA_Child1_L1_ID,
		SideA_Child1_L1.ParentID as  SideA_Child1_L1_StructureID
...
FROM
	graph.[Location]						 as SideA_Child1_L1,
	graph.[StructureAttachLocation] FOR PATH as SideA_Child1_Attach,
	graph.[Location]    FOR PATH             as SideA_L1,
	graph.[LocationLink] FOR PATH			 as SideA_Link,
	graph.[Location]     FOR PATH as SideA_L2,
	graph.[StructureAttachLocation] FOR PATH as SideA_Child2_Attach,
	graph.[Location]     FOR PATH            as SideA_Child2_L1
	/*
...
WHERE 
	   SideA_Child1_L1.ParentID = 125956 AND
	   SideA_Child1_L1.ID in (Select * from @ConnectingChildIDs)
	  AND
	  MATCH(
			SHORTEST_PATH(SideA_Child1_L1     (-(SideA_Child1_Attach)->SideA_L1){1,2} ) AND
			SHORTEST_PATH(LAST_NODE(SideA_L1) (-(SideA_Link)->SideA_L2){1,20} ) AND
			SHORTEST_PATH(LAST_NODE(SideA_L2) (-(SideA_Child2_Attach)->SideA_Child2_L1){1,2} ) --AND
			/*SHORTEST_PATH(LAST_NODE(SideA_
