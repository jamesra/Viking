...
namespace Geometry
{
    ...
    {
        private readonly struct Segment(GridVector2 a, GridVector2 b)
        {
            public GridVector2 A { get; } = a;
            public GridVector2 B { get; } = b;
        }

        /// <summary>
        /// Closed rings in half-pixel coordinates. Vertices sit on cell edges so adjacent pixels share endpoints.
        /// </summary>
        public static IReadOnlyList<GridVector2[]> FindContours(bool[] mask, int width, int height)
        {
            if (mask is null)
        ...
        {
            GridVector2 top = new(x + 0.5, y);
            GridVector2 right = new(x + 1, y + 0.5);
            GridVector2 bottom = new(x + 0.5, y + 1);
            GridVector2 left = new(x, y + 0.5);
        ...
        private static void Add(List<Segment> segments, GridVector2 a, GridVector2 b) =>
            segments.Add(new Segment(a, b));

        private static IReadOnlyList<GridVector2[]> JoinSegments(IReadOnlyList<Segment> segments)
        {
            Dictionary<GridVector2, List<int>> adjacency = [];
            ...
            List<GridVector2[]> contours = [];
            for (int i = 0; i < segments.Count; i++)
            {
                if (used[i])
                    continue;

                List<GridVector2> contour = [segments[i].A, segments[i].B];
                used[i] = true;
                GridVector2 current = segments[i].B;
        ...
        private static void AddAdjacent(Dictionary<GridVector2, List<int>> adjacency, GridVector2 point, int segmentIndex)
        {
