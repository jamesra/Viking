    ...
    public class NeuronJSONView
    {
        ...
        static public NeuronJSONView ToJSON(NeuronGraph graph)
        {
            ...
            foreach (NeuronEdge edge in graph.Edges)
            {
                ...
                string KeyString = SourceNode.Structure.ParentID.ToString() + "-" + TargetNode.Structure.ParentID.ToString() + "," + edge.SynapseType;

                JSONView.edgesJSON.Add(new
                {
                    id = edgeCount,
                    node1 = SourceNode.Key + "(" + SourceNode.Structure.Label + ")",
                    node2 = TargetNode.Key + "( " + TargetNode.Structure.Label + " )",
                    label = KeyString,
