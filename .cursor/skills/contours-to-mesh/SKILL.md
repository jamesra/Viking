...
- **Registration correction** (`--correction` flags, before slicing). `neighbor` is the spatial residual field (leave-one-out predicted−actual, pooled). `curvefit` is a later outlier pass for folds/tears the field is systematically wrong about — not a hole-fill for unsampled points. Default `all`/`everything` runs both in that order. `LimitOffsetToAvoidSameSectionOverlap` halves a curvefit move until it no longer intersects a same-section contour of the same structure. A `same-section shapes a and b intersect` note that disappears with `--correction none` was correction; one that stays is the annotation.
