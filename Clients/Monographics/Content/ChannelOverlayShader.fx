...
PixelShaderOutput HCLToRGBPixelShaderFunction(VertexShaderOutput input)
{
	...
	float4 ChannelColor = tex2D(OverlayTextureSampler, input.TexCoord.xy);
...
{
	//We only use the Luma channel of the background texture.  If the background is grayscale it
	//should not need to be converted to HSV, but if it is not grayscale, you will need to convert it
	// 
...
technique HSVOverBackgroundValueOverlayEffect
{
    pass
    {
