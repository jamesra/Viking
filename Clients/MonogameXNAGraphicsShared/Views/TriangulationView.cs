...
namespace VikingXNAGraphics
{
    public class TriangulationView
    {
        ...
        private void UpdateTriangulationViews(IReadOnlyMesh2D<IVertex2D> mesh)
        {
            try
            {
                ...
                if (mesh.Vertices.Count == 0)
                {
                    ...
                    BoundingBox = new Geometry.Rectangle();
                    return;
                }
                ...
                if ((Points_X.Count != mesh.Vertices.Count) || (Points_X.Points.First() != mesh.Vertices.First().Position))
                {
                    ...
                    Geometry.Rectangle rect = Points_X.BoundingBox();
                    rect = Geometry.Rectangle.Scale(rect, 1.05);
                    BoundingBox = rect;
                }
        ...
        static MeshModel<VertexPositionNormalColor> CreateMeshModel(IReadOnlyMesh2D<IVertex2D> mesh)
        {
            ...
            {
                Verticies = [.. mesh.Vertices.Select(v => new VertexPositionNormalColor((v.Position).ToXNAVector3(0), Vector3.UnitZ, ColorExtensions.Random().SetAlpha(0.5f)))],
        ...
        static MeshModel<VertexPositionNormalColor> CreateMeshModel(IReadOnlyMesh3D<IVertex3D> mesh)
        {
            MeshModel<VertexPositionNormalColor> model = new()
            {
                Verticies = [.. mesh.Vertices.Select(v => new VertexPositionNormalColor(v.Position.ToXNAVector3(), Vector3.UnitZ, ColorExtensions.Random().SetAlpha(0.5f)))],
                Edges = [.. mesh.Faces.SelectMany(f => f.iVerts)]
            };
            return model;
        }
