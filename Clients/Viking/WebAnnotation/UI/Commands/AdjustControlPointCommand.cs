...
namespace WebAnnotation.UI.Commands
{
    internal class AdjustCurveControlPointCommand : AnnotationCommandBase, Viking.Common.IHelpStrings, Viking.Common.IObservableHelpStrings
    {
        ...
        private readonly Geometry.Vector2[] OriginalControlPoints;
        private int iAdjustedControlPoint = -1;

        public delegate void OnCommandSuccess(Geometry.Vector2[] VolumeControlPoints, Geometry.Vector2[] MosaicControlPoints);
        ...
        private void CreateView(Geometry.Vector2[] ControlPoints, Microsoft.Xna.Framework.Color color, double LineWidth, bool IsClosed)
        {
        ...
        protected virtual void UpdatePosition(Geometry.Vector2 PositionDelta) => curveView.SetPoint(iAdjustedControlPoint, curveView.ControlPoints[iAdjustedControlPoint] + PositionDelta);

        protected void PopulateControlPointIndexIfNeeded(Geometry.Vector2 WorldPosition)
        {
            if (iAdjustedControlPoint < 0)
            {
                double[] DistanceArray = [.. curveView.ControlPoints.Select(p => Geometry.Vector2.Distance(p, WorldPosition))];
        ...
        protected override void Execute()
        {
            ...
            {
                Geometry.Vector2[] TranslatedOriginalControlPoints;
                Geometry.Vector2[] MosaicControlPoints = null;

                if (curveView.TryCloseCurve)
                {
                    List<Geometry.Vector2> LoopedPointsList = [.. curveView.ControlPoints];
