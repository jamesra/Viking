...
//        public RefreshDelegate RefreshMethod; 
        ...
        private void HandleWebResponse(HttpWebResponse response)
        {
            lock (this)
            {
                ...
                try
                {
                    ...
                    {
                        Texture2D tex = Texture2D.FromStream(this.graphicsDevice, memStream);
                        this.SetTexture(tex);
                    }
            ...
            lock (this)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        using (MemoryStream memstream = new MemoryStream(requestState.databuffer))
                        {
                            Texture2D tex = Texture2D.FromStream(graphicsDevice, memstream);
                            this.SetTexture(tex);
                        }
                ...
                lock (this)
                {
                    ...
                    try
                    {
                        ...
                        if (texStream != null)
                        {
                            ...
                            try
                            {
                                SetTexture(TextureFromStream(texStream, this.MipMapLevels > 0));
     ...
     */
        ...
        public Texture2D TextureFromStream(Stream texStream, bool mipmap)
        {
            try
            {
                Texture2D tex = Texture2D.FromStream(this.graphicsDevice, texStream);
                return tex;
            }
            catch(InvalidOperationException e)
            {
        ...
        public Texture2D TextureFromData(TextureData texdata, bool mipmap)
        {
            ...
            try
            {
                ...
                {
                    tex = new Texture2D(graphicsDevice, texdata.width, texdata.height, mipmap, SurfaceFormat.Color);
                    tex.SetData<int>(Array.ConvertAll<byte, int>(texdata.pixelBytes, new Converter<byte, int>((x) => (int)x << 16)));
